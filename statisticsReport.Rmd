...
This statistics report is a supplement of our preprint article by @AranbarriEtAl2025ComplexityReducesFeeding in which we investigate how habitat affects the strength of the feeding functional response of two freshwater invertebrate predators. Find below information on related works and the location of the data and code.
...
saveRDS(phen_tests, file = "results_type/phen_tests.rds")
```

```{r phenTable}
phen_type_table(
  phen_test_results = readRDS(here::here("results_type", "phen_tests.rds")),
...
saveRDS(gen_fr_results, file = "results_type/gen_fr_results.rds")
```

```{r genFRtable}
gen_fr_table(
  gen_fr_results = readRDS(here::here("results_type", "gen_fr_results.rds")),
...
We will compare all models using common information criteria, the AIC, and the BIC [@Akaike1974NewLookStatistical; @Schwarz1978EstimatingDimensionModela]. The AIC selects models with more parameters, whereas the BIC tends to select models with a lower number of parameters [@Schwarz1978EstimatingDimensionModela]. Please find the results of all fits saved as `*.rds` files in the folders `results_hab_ie` and `results_hab_ng` online (<https://github.com/b-c-r/CRITTERstatistics>).

```{r run_h_tests, eval=evaluate_selected_chunks, message=FALSE, warning=FALSE}
## receiving source file names
source_files <- list.files(here::here("scripts"))

## setup the cluster for parallel computing on the local machine
cl <- parallel::makeCluster(8)
doParallel::registerDoParallel(cl)

## run all the different model fits in parallel 
foreach::foreach(
  i = 1:length(source_files)
) %dopar% {
  source(here::here("scripts", source_files[i]))
}
...
```{r IeAicBicTable}
result_ie_files <- list.files(here::here("results_hab_ie"))
results_hab_ie <- foreach::foreach(i = 1:length(result_ie_files)) %do%{
  readRDS(here::here("results_hab_ie", result_ie_files[i]))
}
...
```{r NgAicBicTable}
result_ng_files <- list.files(here::here("results_hab_ng"))
results_hab_ng <- foreach::foreach(i = 1:length(result_ng_files)) %do%{
  readRDS(here::here("results_hab_ng", result_ng_files[i]))
}
...
out <- foreach::foreach(i = 1:32, .combine = "rbind") %do% {
  ...
  ci_samples <- MASS::mvrnorm(10000, mu = results_hab_ie[[i]]@coef, Sigma = bbmle::vcov(results_hab_ie[[i]]))
  
  par_exp = 10^bbmle::summary(results_hab_ie[[i]])@coef[,1]
  ...
  data.frame(
    Parameter = rownames(bbmle::summary(results_hab_ie[[i]])@coef),
    PointEstimate = round(bbmle::summary(results_hab_ie[[i]])@coef[,1],3),
    SE = round(bbmle::summary(results_hab_ie[[i]])@coef[,2],3),
    z = round(bbmle::summary(results_hab_ie[[i]])@coef[,3],3),
    p = round(bbmle::summary(results_hab_ie[[i]])@coef[,4],4),
...
#non_exp <- c( 8,25,34,37,41,43,45,70,88,105,114,117,121,123,125,150)
...
out %>%
  knitr::kable(
    escape = TRUE,
    format = "latex",
    caption = "All 32 summary tables from \\textit{Ischnura elegans} model fits.",
    booktabs = TRUE,
    longtable = TRUE,
    col.names = c(
      "Name",
...
out <- foreach::foreach(i = 1:32, .combine = "rbind") %do% {
  ...
  ci_samples <- MASS::mvrnorm(10000, mu = results_hab_ng[[i]]@coef, Sigma = bbmle::vcov(results_hab_ng[[i]]))
  
  par_exp = 10^bbmle::summary(results_hab_ng[[i]])@coef[,1]
  par_value_low <- foreach::foreach(j = 1:ncol(ci_samples), .combine = "c") %do% {10^as.numeric(quantile(ci_samples[,j], 0.025))}
  par_value_up  <- foreach::foreach(j = 1:ncol(ci_samples), .combine = "c") %do% {10^as.numeric(quantile(ci_samples[,j], 0.975))}
  
  data.frame(
    Parameter = rownames(bbmle::summary(results_hab_ng[[i]])@coef),
    PointEstimate = round(bbmle::summary(results_hab_ng[[i]])@coef[,1],3),
    SE = round(bbmle::summary(results_hab_ng[[i]])@coef[,2],3),
    z = round(bbmle::summary(results_hab_ng[[i]])@coef[,3],3),
    p = round(bbmle::summary(results_hab_ng[[i]])@coef[,4],4),
    ParameterExp = round(par_exp,3),
    par_value_low = round(par_value_low,3),
    par_value_up = round(par_value_up,3),
    row.names = NULL
...
```{r packageInfo}
dplyr::data_frame(
  pname = sessioninfo::package_info(pkgs = "loaded", include_base = TRUE, dependencies = TRUE)[,1],
  pver = sessioninfo::package_info(pkgs = "loaded", include_base = TRUE, dependencies = TRUE)[,3],
  pattached = sessioninfo::package_info(pkgs = "loaded", include_base = TRUE, dependencies = TRUE)[,6],
  pbase = sessioninfo::package_info(pkgs = "loaded", include_base = TRUE, dependencies = TRUE)[,7]
) %>%
  dplyr::arrange(
    desc(pbase),
    desc(pattached)
  ...
  knitr::kable(
    escape = TRUE,
    format = "latex",
    caption = "All loaded R packages that we used in this report, including base packages.",
    booktabs = TRUE,
    longtable = TRUE,
    col.names = c(
      "Package Name",

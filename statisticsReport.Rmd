...
evaluate_selected_chunks <- TRUE                                                # set to TRUE for a version to publish
ci_runs <- 10000                                                                # choose 10000 for publication output
threads_gen_fr <- 10                                                            # number of threads that should be used for generalized FR fitting
threads_hab_stats <- 16                                                         # number of threads that should be used for habitat statistics fitting
threads_plots <- 20                                                             # number of threads that should be used for plotting functions to create CI bands
```
...
We used R Markdown [@xieMarkdownDefinitiveGuide2018; @allaireRmarkdownDynamicDocuments2024; @xieMarkdownCookbook2020] with bookdown [@Xie2025BookdownAuthoringBooks; @xieBookdownAuthoringBooks2016] and TinyTex [@XiePositSoftwarePBC2025TinytexHelperFunctions] to create the pdf report. Please follow the TinyTex installation guidelines on [their homepage](https://yihui.org/tinytex/). You can, of course, use another LaTeX distribution, but we only tested if the R Markdown script works using TinyTex.
...
We found four more models within a range of two delta units of both information criteria, all with three to four parameters (table \ref{tab:IeAicBicTable}), and we will discuss these models below. Please find the full table \ref{tab:IeAicBicTableAll} in the section "[Appendix]" below.
